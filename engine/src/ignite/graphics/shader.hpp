...
namespace ignite
{
    ...
    public:
        Shader() = default;
        Shader(nvrhi::IDevice *device, const std::filesystem::path &filepath, ShaderStage stage);

        static ShaderData CompileOrGetVulkanShader(const std::filesystem::path &filepath, ShaderStage stage);
        ...
        static Ref<Shader> Create(nvrhi::IDevice *device, const std::filesystem::path &filepath, ShaderStage stage);
